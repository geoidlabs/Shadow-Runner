...
void checkItems()
{
	if (geo.framesElapsed(1))
	{
		...
		if (itemX[ITEM_BIRD_TWO] < -127)
		{
			...
			if ((showitems & B00001010) == B00001010)
				showitems ^= B00001000; // if stone2 no Bird2
...
void drawItems()
{
	if (showitems & B00000001)
		sprites.drawPlusMask(itemX[ITEM_STONE_ONE], 36, stone_plus_mask, 0);
	if (showitems & B00000010)
		sprites.drawPlusMask(itemX[ITEM_STONE_TWO], 36, stone_plus_mask, 0);
	if (showitems & B00000100)
		sprites.drawErase(itemX[ITEM_BIRD_ONE], 16, bird, birdFrame);
	if (showitems & B00001000)
		sprites.drawErase(itemX[ITEM_BIRD_TWO], 16, bird, birdFrame);
	if (showitems & B00100000)
		sprites.drawErase(itemX[ITEM_EXTRA_LIFE], 4, heart, heartFrame);
}
